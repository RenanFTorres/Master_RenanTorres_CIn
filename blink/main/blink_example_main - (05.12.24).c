...
esp_err_t send_web_page(httpd_req_t *req)
{
    ...
    sprintf(response_data, html_page, energia, potencia); //montar pagina web com html e variaveis
...
//     ESP_LOGI(TAG, "Handler foi chamado!");
//     const char resp[] = "<html><body><h1>Servidor HTTP Funcionando!</h1></body></html>";
...
// // Função de configuração do servidor HTTP
// esp_err_t start_webserver(void)
// {
//     httpd_config_t config = HTTPD_DEFAULT_CONFIG();  // Usando a configuração padrão para o servidor HTTP
//     esp_err_t err = httpd_start(&server, &config);  // Inicia o servidor HTTP com as configurações definidas

//     if (err == ESP_OK) {
//         // Configura o manipulador para o caminho principal ("/")
//         httpd_uri_t uri_get = {
//             .uri = "/",
//             .method = HTTP_GET,
//             .handler = get_req_handler,  // A função que será chamada quando houver uma requisição GET
//             .user_ctx = NULL
//         };
//         httpd_register_uri_handler(server, &uri_get);  // Registra o manipulador de URI

//         ESP_LOGI(TAG, "Servidor Web iniciado");
//     }

//     return err;  // Retorna o erro ou sucesso na inicialização
// }
...
void hello_task_core_1(void *pvParameter)
{
	...
	while(1){    
        ...
        //TENSAO ELETRICA
            ...
            //printf("%.2d/%.2d/%.2d %.2d:%.2d:%.2d\n", timeinfo.tm_mday,timeinfo.tm_mon+1,timeinfo.tm_year+1900,timeinfo.tm_hour,timeinfo.tm_min,timeinfo.tm_sec);//Formatação do tempo e impressão como: dia/mês/ano hora:minuto:segundo
...
//            printf("%.6f\n",current/1000);
        ...
        //printf("%.6f\n",tempo);
        tempo /= 3600;
        energia += potencia*tempo*0.92; // Os valores eficazes são calculados a cada 12 ciclos. 1 ciclo dura 16,67ms, logo, os valores eficazes sao coletados durante 200ms.
        // 0.92 é o fator de potência teórico utilizado
        // time(&now); //Função que retorna o valor do tempo em segundos desde a Epoch
        // localtime_r(&now, &timeinfo); //Função que converte o valor do tempo em segundos desde a Epoch contido em "now" e armazena os dados detalhados de tempo em "timeinfo"
        // printf("%.2d/%.2d/%.2d %.2d:%.2d:%.2d\n", timeinfo.tm_mday,timeinfo.tm_mon+1,timeinfo.tm_year+1900,timeinfo.tm_hour,timeinfo.tm_min,timeinfo.tm_sec);//Formatação do tempo e impressão como: dia/mês/ano hora:minuto:segundo
        print_current_time(); // Atualiza e imprime o tempo atual
        //printf("Parada de 5 segundos\n");
...
//         }
        ...
        if (afundamento == 1 && flag0 == 0) {
            printf("Afundamento de tensão detectado!\n");
            flag0 = 1;
            //vTaskDelay(5000 / portTICK_PERIOD_MS); // Aguardar 1 segundo
        }
        
        if (afundamento == 0 && flag0 == 1) {
            printf("\n");
            printf("Afundamento de tensão mitigado!\n");
            printf("\n");
            flag0 = 0;
            //vTaskDelay(5000 / portTICK_PERIOD_MS); // Aguardar 1 segundo
        }

        if (interrupcao == 1 && flag1 == 0) {
            printf("\n");
            printf("Falta de energia detectada!\n");
            printf("\n");
        ...
        if (interrupcao == 0 && flag1 == 1) {
            printf("\n");
            printf("Falta de energia mitigada!\n");
            printf("\n");
            flag1 = 0;
            //vTaskDelay(5000 / portTICK_PERIOD_MS); // Aguardar 1 segundo
        }

        if (sobretensao == 1 && flag2 == 0) {
            printf("\n");
            printf("Sobretensão detectada!\n");
            printf("\n");
            flag2 = 1;
            //vTaskDelay(5000 / portTICK_PERIOD_MS); // Aguardar 1 segundo
        }
        
        if (sobretensao == 0 && flag2 == 1) {
            printf("\n");
            printf("Sobretensão mitigada!\n");
            printf("\n");
            flag2 = 0;
            //vTaskDelay(5000 / portTICK_PERIOD_MS); // Aguardar 1 segundo
        }

        if (!(transporte == TRANSPORT_EMPTY)) {
            if (flag4 == 1) {
                energia_hr = transporte;
                printf("\n");
                printf("Valor acumulado da energia consumida na última hora foi obtido: %.4f Wh\n", energia_hr);
                printf("\n");
                flag4 = 0;
            }
            if (timeinfo.tm_sec > 00 && timeinfo.tm_min == 00 ) {
                flag3 = 1;
                transporte = TRANSPORT_EMPTY;
            }
            //vTaskDelay(5000 / portTICK_PERIOD_MS); // Aguardar 1 segundo
        }
