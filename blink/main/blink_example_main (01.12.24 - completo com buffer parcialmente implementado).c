...
// Declaração dos buffers de transporte
CIRCULAR_BUFFER_DECLARE(energia_buffer);
...
//            printf("%.6f\n",current/1000);
        ...
        //vTaskDelay(2579 / portTICK_PERIOD_MS);
        ...
        buffer_insert(&energia_buffer,&energia, 1);
...
void hello_task_core_0(void *pvParameter)
{
    ...
    // Return latest conversion value
            ...
            while(buffer_empty(&energia_buffer) == 0) {
    
                buffer_retrieve(&energia_buffer, &saida, 1);
                printf("%hu",&saida );
